...
import static class23.ArrayPrinter2D.print;

// 本题测试链接 : https://leetcode.com/problems/minimum-cost-to-merge-stones/
public class Code05_MinimumCostToMergeStonesEdition1 {
...
//	}
    ...
    public static int mergeStones2(int[] stones, int K) {
        ...
        int ans = process2(0, n - 1, 1, stones, K, presum, dp);
        return ans;
    }
    ...
    public static int process2(int L, int R, int P, int[] arr, int K, int[] presum, int[][][] dp) {
        ...
        int ans = Integer.MAX_VALUE;
        if (P == 1) {
            ans = ((R - L + 1) == K ? 0 : process2(L, R, K, arr, K, presum, dp)) + presum[R + 1] - presum[L];
        } else {
            for (int mid = L; mid < R; mid += K - 1) {
...
//            int[] arr = {1, 10, 5, 2, 10, 2};
//            int K = 2;
            int[] arr = {9, 0, 10, 7, 4, 2, 7};
            int K = 3;
    ...
    /**
     int L= 0;
     int R= arr.length;
     int[] arr1 = Arrays.copyOfRange(arr,L,R-1);
     int ans = mergeStones2(arr1, K);
     */
