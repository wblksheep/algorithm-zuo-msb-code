...
public class Code03_NotContains4Edition1 {
    ...
    public static long notContains4Nums2(long num) {
        int len = getLen(num);
        long tmp1 = powOfBase(len - 1);
        return process(num, tmp1, len);
    }

    public static long process(long num, long tmp1, int len) {
        if (num < 1) {
            return 0;
        }
        int firstNum = (int) (num / tmp1);
        long firstSize = firstNum < 4 ? firstNum * arr[len] : (firstNum - 1) * arr[len];
        return firstSize + process(num % tmp1, tmp1 / 10, len - 1);
    }

    public static long process2(long num) {
        if (num < 1) {
            return 0;
        }
        int len = getLen(num);
        ...
        int firstNum = (int) (num / tmp1);
        long firstSize = firstNum < 4 ? firstNum * arr[len] : (firstNum - 1) * arr[len];
        return firstSize + process2(num % tmp1);
    }

    public static long powOfBase(int len) {
